...
public class Index {
	...
	/**
	 * Take a word and only keep is racine For example : discrimination ->
	 * discriminer : discrinaient -> discriminaient
	 *
	 * @param words
	 */
	 ...
	 * @return a map that associate a token to the number of time it waw
	...
	public static DbInRam createDbInRam(List<ParagraphBrut> words) throws FileNotFoundException, IOException {
		...
		for (ParagraphBrut paraBrut : words) {
			...
			for (TermeParagraphe termeParagraphe : mapTermPara.values()) {
				for(ApparitionType apparitionType : ApparitionType.values()) {
					if(termeParagraphe.hasApparitionType(apparitionType)) {
						termeParagraphe.setTf(termeParagraphe.getTf()*apparitionType.getPonderation());
					}
				}
			}
		}
	...
	private static void updateSets(DbInRam result, Map<String, Terme> mapTermes,
		...
		for (String token : tokens) {
			...
			termeParagraphe.addApparitionTypes(apparitionType);
	 ...
	 *
	 * @param text
	 * @return return a map with all token found in the text and there
	 * occurence, token only contain ascii password and are stemmed,
	...
	/**
	 * Prends un ensemble de map token occurence et le merge avec des coeffs
	 * spécifique
	 *
	 * @param coeffs
	 * @param maps
